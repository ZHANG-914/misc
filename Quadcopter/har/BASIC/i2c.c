...
{ //I2C接口初始化
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_I2C3, ENABLE); //启动I2C功能
...
{									 //I2C发送数据串（器件地址，内部地址，寄存器，数量）
	I2C_GenerateSTART(I2C3, ENABLE); //产生起始位
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;															 //清除EV5
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Transmitter); //发送器件地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
		;						   //清除EV6
	I2C_SendData(I2C3, WriteAddr); //内部功能地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		; //移位寄存器非空，数据寄存器已空，产生EV8，发送数据到DR既清除该事件
	while (NumByteToWrite--)
	{								  //循环发送数据
		I2C_SendData(I2C3, *pBuffer); //发送数据
		pBuffer++;					  //数据指针移位
		while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
			; //清除EV8
	}
	I2C_GenerateSTOP(I2C3, ENABLE); //产生停止信号
}
void I2C_SAND_BYTE(u8 SlaveAddr, u8 writeAddr, u8 pBuffer)
{									 //I2C发送一个字节（从地址，内部地址，内容）
	I2C_GenerateSTART(I2C3, ENABLE); //发送开始信号
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;															 //等待完成
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Transmitter); //发送从器件地址及状态（写入）
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
		;						   //等待完成
	I2C_SendData(I2C3, writeAddr); //发送从器件内部寄存器地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		;						 //等待完成
	I2C_SendData(I2C3, pBuffer); //发送要写入的内容
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		;							//等待完成
	I2C_GenerateSTOP(I2C3, ENABLE); //发送结束信号
}
...
{ //I2C读取数据串（器件地址，寄存器，内部地址，数量）
	while (I2C_GetFlagStatus(I2C3, I2C_FLAG_BUSY))
		;
	I2C_GenerateSTART(I2C3, ENABLE); //开启信号
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;															 //清除 EV5
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Transmitter); //写入器件地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
		; //清除 EV6
	I2C_Cmd(I2C3, ENABLE);
	I2C_SendData(I2C3, readAddr); //发送读的地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		;							 //清除 EV8
	I2C_GenerateSTART(I2C3, ENABLE); //开启信号
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;														  //清除 EV5
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Receiver); //将器件地址传出，主机为读
	...
	while (NumByteToRead)
	{
		if (NumByteToRead == 1)
		{										  //只剩下最后一个数据时进入 if 语句
			I2C_AcknowledgeConfig(I2C3, DISABLE); //最后有一个数据时关闭应答位
			I2C_GenerateSTOP(I2C3, ENABLE);		  //最后一个数据时使能停止位
		}
		if (I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_RECEIVED))
		{									  //读取数据
			*pBuffer = I2C_ReceiveData(I2C3); //调用库函数将数据取出到 pBuffer
			pBuffer++;						  //指针移位
...
{ //I2C读取一个字节
	...
	while (I2C_GetFlagStatus(I2C3, I2C_FLAG_BUSY))
		;							 //I2C等待总线不忙
	I2C_GenerateSTART(I2C3, ENABLE); //产生一个起始信号
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;															 //等待事件EV5，即起始信号已发送完毕
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Transmitter); //发送读写地址，并设置传输方向为写入
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
		;						  //等待从设备应答，产生EV6，EV8事件
	I2C_Cmd(I2C3, ENABLE);		  //使能I2C总线
	I2C_SendData(I2C3, readAddr); //发送从设备的寄存器地址
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		;							 //等待响应，等待事件EV8_2
	I2C_GenerateSTART(I2C3, ENABLE); //重起始信号
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT))
		;														  //等待起始信号发送完毕，产生事件EV5
	I2C_Send7bitAddress(I2C3, SlaveAddr, I2C_Direction_Receiver); //发送读写地址，并设置传输方向为读取
	while (!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))
		;								  //等待从设备应答，产生事件6
	I2C_AcknowledgeConfig(I2C3, DISABLE); //关闭应答位
	I2C_GenerateSTOP(I2C3, ENABLE);		  //发送停止信号
