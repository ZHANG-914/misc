...
static void Task_COM(void* p_arg)
{
    int32_t temp;
    while (1) {
        ANO_DT_Send_Senser(acc.x, acc.y, acc.z, gyro.x , gyro.y , gyro.z , mag.x, mag.y, mag.z); //发送传感器原始数据帧
...
//            AHRSupdate(fGyro.x, fGyro.y, fGyro.z, acc.x, acc.y, acc.z, mag.x, mag.y, mag.z);
